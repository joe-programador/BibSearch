...
//        ClassLs += ' (<' + ResqLis[oneRes].resourceClass + '>  <' + ResqLis[oneRes].labelProperty + '>  )';
        for (var oneProp = 0; oneProp < ResqLis[oneRes].indexProperties.length; oneProp++) {
          ClassIndxLs += '( <' + ResqLis[oneRes].resourceClass + '>  <' + ResqLis[oneRes].labelProperty + '> <' + ResqLis[oneRes].indexProperties[oneProp] + '> \'' + ResqLis[oneRes].indexPropertiesName[oneProp] + '\' \'' + ResqLis[oneRes].indexPropertiesName[oneProp] + ':(' + TextSearch + ')\' )  ';

//                                    Query += "      bind (concat (?PropertyLabel, ':("+TextSearch+")' ) as ?stx ) .";
        }
      }
...
//            Query += "      (?EntityURI ?Score ?PropertyValue) text:query (?Property '("+TextSearch+")' 1000) .\n";
      ...
      //Query += "{ select ?Property ?PropertyLabel ?EntityURI ?Score ?PropertyValue { ";

      Query += "      values (?EntityClass ?EntityLabelProp ?Property ?PropertyLabel ?stx) {\n";
      Query += ClassIndxLs + "\n";
      Query += "      } .\n";
      ...
      Query += "      ?EntityURI :snippets _:s .";
      Query += "      _:s :snippetText ?PropertyValue .";
